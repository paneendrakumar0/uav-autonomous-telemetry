#!/usr/bin/env python3
"""Randomized evaluation of the frozen crosswind-rejection candidate."""
...
def compare(trial_df, args):
    ...
    for speed_index, speed in enumerate(args.wind_speeds):
        ...
        for metric_index, metric in enumerate(METRICS):
            ...
            candidate = speed_group[speed_group["profile"] == "geometric"][metric].astype(float)
...
def write_report(out_dir, args, aggregate_df, comparison_df):
    ...
    report = f"""# Frozen Crosswind Candidate Validation - {date.today().isoformat()}
...
"""
    (out_dir / "FROZEN_CROSSWIND_VALIDATION_SUMMARY.md").write_text(report)


def main():
    parser = argparse.ArgumentParser(description="Run randomized validation of the frozen crosswind candidate.")
    ...
    parser.add_argument("--run-order-seed", type=int, default=20_260_803)
    ...
    parser.add_argument("--out-dir", default="reports/crosswind_disturbance_rejection_v1/frozen_validation")
    ...
    trial_df.to_csv(out_dir / "crosswind_trial_metrics.csv", index=False)
    aggregate_df.to_csv(out_dir / "crosswind_aggregate_metrics.csv", index=False)
    comparison_df.to_csv(out_dir / "crosswind_statistical_comparison.csv", index=False)
    write_report(out_dir, args, aggregate_df, comparison_df)
    write_manifest(
        out_dir,
        experiment_type="frozen_crosswind_candidate_validation",
        repo_root=repo_root,
        px4_dir=px4_dir,
        parameters={
        ...
        data={
            ...
            "trial_metrics_csv": "crosswind_trial_metrics.csv",
            "aggregate_metrics_csv": "crosswind_aggregate_metrics.csv",
            "statistical_comparison_csv": "crosswind_statistical_comparison.csv",
            "summary_markdown": "FROZEN_CROSSWIND_VALIDATION_SUMMARY.md",
        },
        result={
    ...
    print(f"Frozen crosswind validation artifacts written to {out_dir}")
