...
def write_experiment_manifest(out_dir, args, repo_root, px4_dir, summary):
    write_manifest(
        out_dir,
        experiment_type="single_payload_validation",
        repo_root=repo_root,
        px4_dir=px4_dir,
        parameters={
...
def main():
    ...
    try:
        ...
        if args.profile == "geometric":
            ...
            launch_args.extend(
                [
                    f"ki_xy:={args.geometric_ki_xy}",
                    f"ki_z:={args.geometric_ki_z}",
                    f"integral_limit_xy:={args.geometric_integral_limit_xy}",
                    f"integral_limit_z:={args.geometric_integral_limit_z}",
                    f"integrator_leak_rate:={args.geometric_integrator_leak_rate}",
