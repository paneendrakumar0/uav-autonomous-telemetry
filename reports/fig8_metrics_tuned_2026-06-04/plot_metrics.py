...
def plot_xy(rows, filename="actual_vs_reference_xy.png", title="Figure-8 Actual vs Reference: X-Y"):
...
def write_report(rows):
    ...
    with (BASE / "METRICS.md").open("w") as f:
        ...
        f.write("- `actual_vs_reference_xy_steady.png`\n")
...
def main():
    ...
    plot_xy(rows)
    plot_xy(
        [row for row in rows if row["t_s"] >= 12.0],
        filename="actual_vs_reference_xy_steady.png",
        title="Figure-8 Actual vs Reference: X-Y, t >= 12 s",
    )
    plot_xyz_time(rows)
    plot_error(rows)
    write_report(rows)
